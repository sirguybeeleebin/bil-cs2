...
class ColumnSelectorArray(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X: np.ndarray, y=None) -> ColumnSelectorArray:
        log.debug(f"ColumnSelectorArray: fit на {X.shape[0]} строках")
        return self

    def transform(self, X: np.ndarray, y=None) -> np.ndarray:
        log.debug(f"ColumnSelectorArray: transform с колонками {self.columns}")
...
class BagEncoder(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X: np.ndarray, y=None) -> csr_matrix:
        ...
        log.debug(f"BagEncoder: трансформация массива размером {X.shape}")
...
class PlayerEloEncoder(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> PlayerEloEncoder:
        ...
        log.info(f"PlayerEloEncoder: обучение на {X.shape[0]} матчах")
    ...
    def transform(self, X: np.ndarray) -> np.ndarray:
        log.debug(f"PlayerEloEncoder: трансформация массива размером {X.shape}")
