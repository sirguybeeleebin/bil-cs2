...
class ServiceRepository:
    ...
    async def upsert_service(self, client_id: str, client_secret: str) -> dict | None:
        query = """
        INSERT INTO services (client_id, client_secret, created_at, updated_at)
        VALUES ($1, $2, NOW(), NOW())
        ...
        try:
            async with self.pool.acquire() as conn:
                row = await conn.fetchrow(query, client_id, client_secret)
            return dict(row)
    ...
    async def get_service_by_client_id(self, client_id: str) -> dict | None:
        query = """
        SELECT id, client_id, client_secret, created_at, updated_at 
        FROM services WHERE client_id = $1
        """
        try:
            async with self.pool.acquire() as conn:
